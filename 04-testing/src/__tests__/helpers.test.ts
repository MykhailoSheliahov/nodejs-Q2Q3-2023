...
describe("#helpers", () => {
  describe("validateInput", () => {
    ...
    it("should return error if country is not valid", () => {
      ...
      expect(() => validateInput({ country: "UK" })).toThrow("Country provided is not supported, received: UK")
    });

    it("should return error if year is not valid", () => {
      expect(() => validateInput({ year: 2022 })).toThrow(Error)
      expect(() => validateInput({ year: 2022 })).toThrow("Year provided not the current, received: 2022")
    });

    it("should return error if country is not valid and year is valid", () => {
      expect(() => validateInput({ country: "UK", year })).toThrow(Error)
      expect(() => validateInput({ country: "UK", year })).toThrow("Country provided is not supported, received: UK")
    });
    ...
    it.each([
      ...
      { country: "DE", },
      { country: "NL", },
    ])("should return error if country is valid and year is not valid", ({ country }) => {
      expect(() => validateInput({ country, year: 2022 })).toThrow(Error)
      expect(() => validateInput({ country, year: 2022 })).toThrow("Year provided not the current, received: 2022")
    });
  });
